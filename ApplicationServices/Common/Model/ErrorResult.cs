...
namespace ApplicationServices.Common.Model
{
    public class ErrorResult
    {
        ...
        {
            return new ErrorResult<T>(default, false, message);
        }

        public static ErrorResult<T> Fail<T>(string message, string responseCode)
        {
            return new ErrorResult<T>(default, false, message, responseCode);
        }
    ...
    public class ErrorResult<T> : ErrorResult
    {
        ...
        {
            var result = Fail("An error occured", "", "400");
            var errors = context.ModelState.AsEnumerable();
            foreach (var error in errors)
            {
                foreach (var innerError in error.Value!.Errors)
